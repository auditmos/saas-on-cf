...
function DirectUpdatePage() {
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async (newData) => {
      ...
      const previousClient = queryClient.getQueryData<Client | null>(clientKeys.detail(clientId!, 'direct'));
  ...
  const searchForm = useForm({
    ...
    onSubmit: ({ value }) => {
      if (value.searchId && value.searchId !== clientId) {
        navigate({ to: '/dashboard/direct/update', search: { clientId: value.searchId, editing: false } });
      }
    },
  });
                ...
                <form onSubmit={(e) => { e.preventDefault(); form.handleSubmit(); }} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => navigate({ to: '/dashboard/direct/update', search: { clientId, editing: false } })}
