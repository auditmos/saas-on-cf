...
const operations = [
  ...
  { label: 'Read', href: '/dashboard/direct/read' as const, desc: 'Fetch single client via getClient()' },
  { label: 'List', href: '/dashboard/direct/list' as const, desc: 'Paginated clients via getClients()' },
...
Neon Postgres`}
          ...
          <div className="bg-info/10 p-4 rounded">
            ...
            <p className="text-sm mt-1 text-muted-foreground">Performance-critical reads, complex transactions, SSR required</p>
          </div>

          <div className="bg-warning/10 p-4 rounded">
            <h4 className="font-semibold text-foreground">When NOT to Use</h4>
            <p className="text-sm mt-1 text-muted-foreground">Need shared validation with external API, rate limiting required</p>
          </div>
        </CardContent>
      </Card>
