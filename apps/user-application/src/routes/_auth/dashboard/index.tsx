...
const directOps = [
  { label: 'Create', href: '/dashboard/direct/create' as const, desc: 'Insert new client via createClient()' },
  { label: 'Read', href: '/dashboard/direct' as const, desc: 'Fetch single client via getClient()' },
  { label: 'Update', href: '/dashboard/direct/update' as const, desc: 'Modify client via updateClient()' },
  { label: 'Delete', href: '/dashboard/direct/delete' as const, desc: 'Remove client via deleteClient()' },
];

const bindingOps = [
  { label: 'Create', href: '/dashboard/binding/create' as const, desc: 'POST /clients via service binding' },
  { label: 'Read', href: '/dashboard/binding' as const, desc: 'GET /clients/:id via service binding' },
  { label: 'Update', href: '/dashboard/binding/update' as const, desc: 'PUT /clients/:id via service binding' },
  { label: 'Delete', href: '/dashboard/binding/delete' as const, desc: 'DELETE /clients/:id via service binding' },
];
...
const apiOps = [
  ...
  { label: 'Update', href: '/dashboard/api/update' as const, desc: 'PUT /clients/:id from browser' },
  { label: 'Delete', href: '/dashboard/api/delete' as const, desc: 'DELETE /clients/:id from browser' },
];
...
Neon Postgres`}
              ...
              <div className="space-y-2">
                <div className="bg-muted p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Use when:</span> Performance-critical reads, complex transactions, SSR required</p>
                </div>
                <div className="bg-muted p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Avoid when:</span> Need shared validation with external API, rate limiting required</p>
                </div>
              </div>
            </CardContent>
          </Card>
...
data-ops queries → Neon Postgres`}
              ...
              <div className="space-y-2">
                <div className="bg-muted p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Use when:</span> Internal microservices, shared business logic, centralized validation</p>
                </div>
                <div className="bg-muted p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Avoid when:</span> Need lowest latency, data-service down = app down</p>
                </div>
                <div className="bg-accent/20 p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Service Binding:</span> Hostname in URL is ignored - requests go directly to bound worker via internal network. No CORS, no public latency.</p>
                </div>
              </div>
            </CardContent>
          </Card>
...
data-ops queries → Neon Postgres`}
              ...
              <div className="space-y-2">
                <div className="bg-muted p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Use when:</span> Mobile apps, third-party integrations, public API consumers</p>
                </div>
                <div className="bg-muted p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Avoid when:</span> Need SSR, sensitive operations, internal-only features</p>
                </div>
                <div className="bg-destructive/10 p-3 rounded">
                  <p className="text-sm text-foreground"><span className="font-semibold">Security:</span> API tokens used in browser are visible to users. Use for public data or implement proper authentication for sensitive ops.</p>
                </div>
              </div>
            </CardContent>
          </Card>
