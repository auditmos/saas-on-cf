...
import { z } from 'zod';
import { fetchClient, updateClientApi, ApiError } from '@/lib/api-client';
...
function ApiUpdatePage() {
  ...
  const { data: client, isLoading, error: fetchError, isFetching } = useQuery({
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { name?: string; surname?: string; email?: string }) => updateClientApi(clientId!, data),

    onMutate: async (newData) => {
      await queryClient.cancelQueries({ queryKey: clientKeys.detail(clientId!, 'api') });
      const previousClient = queryClient.getQueryData<Client | null>(clientKeys.detail(clientId!, 'api'));
  ...
  const form = useForm({
    defaultValues: { name: client?.name ?? '', surname: client?.surname ?? '', email: client?.email ?? '' },
  ...
  const searchForm = useForm({
    ...
    onSubmit: ({ value }) => {
      if (value.searchId && value.searchId !== clientId) {
        navigate({ to: '/dashboard/api/update', search: { clientId: value.searchId, editing: false } });
      }
    },
  });
        ...
        <CardContent className="space-y-4">
                ...
                <form onSubmit={(e) => { e.preventDefault(); form.handleSubmit(); }} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => navigate({ to: '/dashboard/api/update', search: { clientId, editing: false } })}
          ...
          {!clientId && (
            <Alert>
              ...
              <AlertDescription>Enter a UUID from the List page to update a client</AlertDescription>
            </Alert>
          )}
        </CardContent>
      </Card>
