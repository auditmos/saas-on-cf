...
import { clientKeys, clientDetailBindingQueryOptions } from '@/lib/query-keys';
...
function BindingUpdatePage() {
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async (newData) => {
      ...
      const previousClient = queryClient.getQueryData<Client | null>(clientKeys.detail(clientId!, 'binding'));
  ...
  const searchForm = useForm({
    ...
    onSubmit: ({ value }) => {
      if (value.searchId && value.searchId !== clientId) {
        navigate({ to: '/dashboard/binding/update', search: { clientId: value.searchId, editing: false } });
      }
    },
  });
                ...
                <form onSubmit={(e) => { e.preventDefault(); form.handleSubmit(); }} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => navigate({ to: '/dashboard/binding/update', search: { clientId, editing: false } })}
